...
def test_backward(device):
    ...
    dt = torch.rand(batch, seqlen, nheads, device=device, requires_grad=True).mul(0.1)
    A = -torch.exp(torch.rand(nheads, dstate, device=device).mul(2))
...
def test_initial_states(device):
    ...
    # initial_states not natively supported by BlellochSSMFn yet (no prepend mechanism)
    pytest.skip("initial_states not yet supported in wrapper")
